import { Bell, Search, AlertCircle, Power, Home, Users, LogOut, ChevronDown, User } from 'lucide-react';
...
export function RiderHeader({ notificationCount, onlineStatus, onToggleStatus, emergencyMode, onToggleEmergency, onGoToLanding, onSwitchRole, onViewProfile, riderName }: RiderHeaderProps) {
  const { refreshTick } = useAutoRefresh();
  const [showProfileMenu, setShowProfileMenu] = useState(false);
  const [showNotifications, setShowNotifications] = useState(false);
  const [inbox, setInbox] = useState<RiderInboxRow[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        channel = supabase
          .channel(`rider_header_inbox_${uid}`)
          .on(
            'postgres_changes',
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-6">
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              {emergencyMode && (
                <span className="flex items-center gap-1 text-red-300 ml-2">
        ...
        <div className="flex-1 max-w-xl mx-6">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search deliveries, patients..."
              className={`w-full pl-12 pr-4 py-2.5 rounded-lg focus:outline-none focus:ring-2 transition ${
                emergencyMode
                  ...
                  : 'border border-gray-300 focus:ring-orange-500 focus:border-transparent'
              }`}
            />
          </div>
        </div>
